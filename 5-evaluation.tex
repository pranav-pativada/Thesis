...
\begin{itemize}
    ...
    \item \textbf{KryBall}: Krylov subspace size $k \in [1, 10]$ under an integer uniform distribution, trust-region radius max $\Delta_{\mathit{max}} \in [1 \times 10^{-2}, 1]$ under a log uniform distribution, krylov refresh rate $r_{\mathit{refresh}} \in [1, 10]$ under an integer uniform distribution.
\end{itemize}
...
We now perform a sensitivity analysis of our method. Our choice of task is the MNIST-1D MLP task, since it is our best performing task. We first perform a sensitivity analysis of the Krylov subspace dimension $M$. We do this by varying $M$ from 1 to 10 and keeping all other hyperparameters constant. Our results are shown in \cref{fig:sens_krylov_dim}. This is followed by a sensitivity analysis of the maximum trust region radius $\Delta_{\mathit{max}}$ and then the krylov refresh rate $r_{\mathit{refresh}}$. We present the results in \cref{fig:sens_max_step} and \cref{fig:sens_r_refresh}.

\begin{figure}[!t]
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/5evals/sens_krylov_dim.png}
        \caption{The sensitivity analysis of the Krylov subspace size $M$ on MNIST-1D MLP.}
        \label{fig:sens_krylov_dim}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figures/5evals/sens_max_step.png}
        \caption{The sensitivity analysis of the maximum trust region radius $\Delta_{\mathit{max}}$ on MNIST-1D MLP.}
        \label{fig:sens_max_step}
    \end{subfigure}
    \vspace{1em}
    \begin{subfigure}[b]{0.49\linewidth}
        \centering
...
\textbf{Maximum Trust Region Radius $(\Delta_{\mathit{max}})$}: The maximum trust region radius 

We see that the maximum trust region radius $\Delta_{\mathit{max}}$ is also sensitive to the performance of KryBall. We see that $\Delta_{\mathit{max}} = 0.3$ is the optimal point, and that $\Delta_{\mathit{max}} = 0.1$ is the worst performing point. This is likely because $\Delta_{\mathit{max}} = 0.1$ is too small and does not provide a good approximation of the landscape, and $\Delta_{\mathit{max}} = 0.5$ is too large and becomes too expensive to compute.
...
In this section, we present critical questions about the results we have observed and the choices we have made in evaluating our algorithm. We then answer these questions with our hypothesis and present further analysis. 

\textbf{Why do we see late epoch instability but good early epoch performance?}We saw in \cref{ssec:results_image_classification} that KryBall achieves good early epoch performance, but late epoch instability. We suspect this has to do with our quadratic approximation. In the early epochs, our quadratic model is accurate and a good approximation of the landscape. However, as training progresses, the landscape changes and we suspect that a quadratic model is less accurate. 

\cite{ma2022beyond} state that the loss landscape of neural networks is multiscale, and that the quadratic model is only a local approximation of the landscape. Specifically, they find that in a neighbourhood of the local minima, the loss mixes a continuum of scales and instead grows subquadratically. As dimensionality increases, they state that the loss landscape instead shows several separate scales. This means that our quadratic model is not accurate in the presence of local minima. Given that the loss landscape is multiscale, and more importantly subquadratic near the local minima, this means that a first-order approximation may be better. In \cref{fig:cifar10_cnn_acc} and \cref{fig:cifar10_resnet_acc}, it is plausible that the variance we see near the end of training is due to this. While we have discussed this in the context of the relevant literature, we also perform our own analysis in \cref{sec:discussion_and_further_analysis}.

\textbf{How good is our approximation of the Hessian?}

\textbf{Are our directions distinct?}
